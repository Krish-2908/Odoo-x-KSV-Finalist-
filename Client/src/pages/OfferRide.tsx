...
const OfferRide: React.FC = () => {
    ...
    const [pickupAddress, setPickupAddress] = useState('Satellite Road, Ahmedabad')
    const [destAddress, setDestAddress] = useState('Odoo House, Gandhinagar')
    const [travelDateTime, setTravelDateTime] = useState('')
    const [availableSeats, setAvailableSeats] = useState(1)
    const [farePerSeat, setFarePerSeat] = useState(10)

    // Coordinate states
    const [pickupCoords, setPickupCoords] = useState<LatLng>({ lat: 23.0225, lng: 72.5714 })
    const [destCoords, setDestCoords] = useState<LatLng>({ lat: 23.2156, lng: 72.6369 })
    ...
    // Forward geocoding function (Address text -> Map Coordinates)
    const handleGeocodeAddress = async (type: 'pickup' | 'dest', addressText: string) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await API.post('/rides', {
                ...
                destination: {
                    address: destAddress,
                    coordinates: [destCoords.lng, destCoords.lat] // [lng, lat]
                },
                travelDateTime,
                availableSeats,
                farePerSeat,
                ...
                {loadingVehicles ? (
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="grid md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onBlur={() => handleGeocodeAddress('pickup', pickupAddress)}
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    e.preventDefault();
                                                    handleGeocodeAddress('pickup', pickupAddress);
                                                }
                                            }}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onBlur={() => handleGeocodeAddress('dest', destAddress)}
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    e.preventDefault();
                                                    handleGeocodeAddress('dest', destAddress);
                                                }
                                            }}
                                ...
                                <div className="space-y-1">
                                    ...
                                    <LocationPickerMap
                                        ...
                                        onRouteCalculated={(dist) => {
                                            ...
                                            const rec = Math.round(dist * 2)
                                            setRecommendedFare(rec)
                                            setFarePerSeat(rec)
                            ...
                            {isRouteConfirmed && (
                                <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl space-y-1.5 text-xs text-slate-700 animate-fade-in">
                                    <p className="font-extrabold text-blue-900 text-sm">✓ Route Synced with Map</p>
                                    <p>🛣️ Total Driving Distance: <span className="font-bold text-slate-800">{routeDistance} km</span></p>
                                    <p>💵 Recommended Fare (₹2/km): <span className="font-bold text-slate-800">₹{recommendedFare}</span></p>
                                </div>
                            )}
